...
export const models: Partial<Record<ModelValues, OptionValue[]>> = {
  ...
  Z3B: [
    ...
    {
      value:
        "https://huggingface.co/TheBloke/stablelm-zephyr-3b-GGUF/resolve/main/stablelm-zephyr-3b.Q3_K_M.gguf",
    ...
    {
      value:
        "https://huggingface.co/TheBloke/stablelm-zephyr-3b-GGUF/resolve/main/stablelm-zephyr-3b.Q8_0.gguf",
...
export async function selectModelUrl(): Promise<string> {
  const modelGroup = await p.group({
    modelName: async () => {
      const [total, available] = await Promise.all([totalmem(), freemem()]);
      p.note(
